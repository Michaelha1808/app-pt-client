...
const bmiLabelColor = computed(() => {
  if (!bmi.value) return '#8E8E93'
  const v = bmi.value.value
  if (v < 18.5) return '#32ADE6'
  if (v < 25)   return '#34C759'
...
<template>
  <div class="pb-24">
    ...
    <div class="flex items-center gap-3 px-4 pt-2 pb-4">
      ...
      <h1 class="text-[17px] font-semibold text-black">Cân nặng</h1>
    </div>
    ...
    <template v-else>
      <!-- Header card -->
      <div class="mx-4 mb-4 bg-gradient-to-br from-ios-blue to-ios-purple rounded-[20px] p-5 text-white">
        <div class="flex items-end justify-between">
          <div>
            <p class="text-[12px] text-white/70 uppercase tracking-wider font-medium">Hiện tại</p>
            <p class="text-[34px] font-bold leading-tight">
              {{ currentWeight ?? '—' }} <span class="text-[16px] font-medium text-white/80">kg</span>
            </p>
            <p v-if="trend" class="text-[13px] text-white/80 mt-1">
              {{ trend.delta_kg <= 0 ? '▼' : '▲' }} {{ Math.abs(trend.delta_kg) }} kg trong {{ range }} ngày
            </p>
          </div>
          <div v-if="bmi" class="text-right">
            <p class="text-[12px] text-white/70">BMI</p>
            <p class="text-[22px] font-bold">{{ bmi.value }}</p>
            <p class="text-[11px] font-semibold" :style="`color: ${bmiLabelColor}`">{{ bmi.label }}</p>
          </div>
        </div>
      </div>

      <!-- Segmented control -->
      <div class="mx-4 mb-4 bg-ios-gray5 rounded-[10px] p-1 flex">
        <button
          v-for="r in [30, 90, 180] as WeightRange[]" :key="r"
          class="flex-1 py-1.5 rounded-[8px] text-[14px] font-semibold transition-all"
          :class="range === r ? 'bg-white text-black shadow-sm' : 'text-ios-gray'"
          @click="selectRange(r)"
        >{{ r }} ngày</button>
      </div>

      <!-- Biểu đồ -->
      <div class="mx-4 mb-4 bg-white rounded-[18px] p-4 shadow-sm">
        ...
        <svg v-else :viewBox="`0 0 ${CHART_W} ${CHART_H}`" class="w-full h-32">
          <polyline :points="avgPoints" fill="none" stroke="#007AFF" stroke-width="1.5" stroke-opacity="0.35" stroke-linejoin="round" stroke-linecap="round"/>
          <polyline :points="rawPoints" fill="none" stroke="#007AFF" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>
          <circle v-if="lastPoint" :cx="lastPoint.x" :cy="lastPoint.y" r="3.5" fill="#007AFF"/>
        </svg>
        <div v-if="trend" class="flex justify-between text-[12px] text-ios-gray mt-2">
          <span>TB 7 ngày: {{ trend.avg_7d_kg }} kg</span>
          <span>Tốc độ: {{ trend.weekly_rate_kg }} kg/tuần</span>
        </div>
      </div>
      ...
      <div class="mx-4 mb-4">
        <button
          class="w-full py-3.5 bg-ios-blue text-white text-[16px] font-semibold rounded-[14px] ios-press"
          @click="openSheet"
        >⚖️ Ghi cân nặng</button>
      </div>

      <!-- Đề xuất mục tiêu -->
      <div v-if="goalSuggestion" class="mx-4 mb-4 bg-ios-orange/10 border border-ios-orange/20 rounded-[18px] p-4">
        <p class="text-[15px] font-semibold text-black mb-1">💡 Đề xuất mục tiêu calo mới</p>
        <p class="text-[13px] text-ios-gray mb-3">{{ goalSuggestion.reason }}. Mục tiêu nên là <b class="text-black">{{ goalSuggestion.suggested_goal }} kcal</b> (hiện tại {{ goalSuggestion.current_goal }} kcal).</p>
        <div class="flex gap-2">
          <button
            class="flex-1 py-2.5 bg-ios-orange text-white text-[14px] font-semibold rounded-[10px] ios-press"
            @click="acceptGoal(goalSuggestion.suggested_goal)"
          >Áp dụng</button>
          <button
            class="flex-1 py-2.5 bg-white text-ios-gray text-[14px] font-semibold rounded-[10px] ios-press"
      ...
      <!-- Danh sách bản ghi -->
      <div class="mx-4">
        <p class="text-[13px] font-semibold text-ios-gray uppercase tracking-wide mb-2 px-1">Lịch sử ghi cân</p>
